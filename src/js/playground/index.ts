 ...
 * to the console.
 */
function setEditorAndConsoleSizes(outputData: boolean = false): void {
	const mediumScreenSize = Number(
		getComputedStyle(codeTextAreaHighlightField).getPropertyValue("--medium-screen-size").replace("px", ""),
	);
	...
	if (outputData) {
		...
		console.log(
			`Code Editor Highlight Field: ${codeTextAreaHighlightField.offsetWidth}x${codeTextAreaHighlightField.offsetHeight}`,
		);
